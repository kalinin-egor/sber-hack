...
export default function App() {
  ...
  const { analyses, loadHistory } = useAudioAnalysis();
  ...
  const handleTranscriptionComplete = (result: AudioAnalysisDto) => {
    // Auto-switch to graph after analysis
    setTimeout(() => setActiveTab("graph"), 1000);
  };

  // Calculate statistics from completed analyses
  const completedAnalyses = analyses.filter(a => a.transcription);
  const averageConfidence = completedAnalyses.length > 0 
    ? completedAnalyses.reduce((acc, a) => acc + (a.transcription?.confidence || 0), 0) / completedAnalyses.length
    : 0;
  const totalDuration = completedAnalyses.reduce((acc, a) => acc + (a.transcription?.duration || 0), 0);
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <Card className="backdrop-blur-sm bg-card/80 shadow-xl border-2">
                      <CardContent className="p-8">
                        ...
                        <div className="space-y-6">
                          {completedAnalyses.slice(-3).reverse().map((analysis, index) => (
                            <motion.div
                              key={analysis.id}
                            ...
                            >
                              <div className="flex items-center justify-between flex-wrap gap-2">
                                <div className="flex items-center gap-3">
                                  ...
                                  >
                                    ✅ {((analysis.transcription?.confidence || 0) * 100).toFixed(1)}% точность
                                  </Badge>
                                </div>
                                <span className="text-sm text-muted-foreground font-medium">
                                  📅 {analysis.transcription?.timestamp}
                                </span>
                              </div>
                              <motion.p
                                initial={{ opacity: 0 }}
                              ...
                              >
                                {analysis.transcription && analysis.transcription.text.length > 200 
                                  ? analysis.transcription.text.substring(0, 200) + '...'
                                  : analysis.transcription?.text
                                }
                              </motion.p>
                              {index < completedAnalyses.slice(-3).length - 1 && (
            ...
            <TabsContent value="graph">
              <motion.div
                ...
                transition={{ duration: 0.5 }}
              >
                <GraphVisualization transcriptionIds={completedAnalyses.map(a => a.id)} />
              </motion.div>
            </TabsContent>
          </Tabs>
        </motion.div>
