...
export function AudioAnalyzer({ onTranscriptionComplete }: { onTranscriptionComplete: (result: TranscriptionResult) => void }) {
  ...
  const analyzeAudio = async () => {
    ...
    const mockResult: TranscriptionResult = {
      text: audioFile ? 
        `Анализ загруженного файла "${audioFile.name}": Это пример транскрипции аудио. Система обработала ваш файл и извлекла текст с высокой точностью. Благодарим за использование нашего сервиса анализа аудио.` :
        "Анализ записанного аудио: Привет! Это тестовая запись, которая была обработана нашей системой анализа речи. Качество записи хорошее, текст распознан успешно.",
      confidence: 0.95,
        ...
        <CardContent className="p-8 space-y-8">
          ...
          >
            <Button 
              ...
              disabled={(!audioFile && !recordedAudio) || isAnalyzing || isRecording}
